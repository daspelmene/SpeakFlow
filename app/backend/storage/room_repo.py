...
class RoomRepository:
    ...
    async def get_pending_invitations(self, user_id: int) -> list[Room]:
        ...
        query = select(Room).where(
            and_(
                ...
                Room.is_invited_accepted.is_(False),
                Room.is_finished.is_(False),
            )
        )
        ...
        query = select(Room).where(
            and_(
                ...
                or_(
                    ...
                    and_(
                        Room.invited_user_id == user_id,
                        Room.is_invited_accepted.is_(True),
                    ),
                ),
            )
        )
    ...
    async def is_user_in_any_room(self, user_id: int) -> bool:
        logger.debug(f"Checking if user {user_id} is in any unfinished room")
        ...
        logger.debug(f"User {user_id} in unfinished room: {in_room}")
    ...
    async def finish_room(self, room_id: UUID) -> bool:
        logger.info(f"Finishing room: {room_id}")
        ...
        if room is None:
            logger.warning(f"Room {room_id} not found for finish")
            return False
        ...
        logger.info(f"Room {room_id} marked as finished")
